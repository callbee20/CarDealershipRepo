...
public class VehicleModelHelper {
	...
	public void deleteVehicle(VehicleModel toDelete) {
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<VehicleModel> typedQuery = em.createQuery(
				"select vm from VehicleModel vm where vm.model =:selectedModel and vm.make =:selectedMake and vm.driveTrain =:selectedDriveTrain and vm.price =:selectedPrice and vm.addedToLot =:selectedAddedToLot",
				VehicleModel.class);
	...
	public List<VehicleModel> searchForVehicleByMake(String make) {
		EntityManager em = emfactory.createEntityManager();
		em.getTransaction().begin();
		TypedQuery<VehicleModel> typedQuery = em
				.createQuery("select vm from VehicleModel vm where vm.make = :selectedMake", VehicleModel.class);
	...
	public List<VehicleModel> searchForVehicleByModel(String model) {
		...
		em.getTransaction().begin();
		TypedQuery<VehicleModel> typedQuery = em
				.createQuery("select vm from VehicleModel vm where vm.model =:selectedModel", VehicleModel.class);
